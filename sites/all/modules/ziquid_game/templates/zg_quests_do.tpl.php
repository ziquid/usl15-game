        ...
        if (($eq_id + $land_id + $st_id) > 0) {
          ...
          if ($eq_id > 0) {
            ...
            if ($eq_success) {
              ...
              zg_slack('could not give 2nd-round eq bonus for quest ' . $game_quest->id .
                " due to $eq_success, $eq_reason, $eq_details",
                $slack_channel);
              $response = zg_slack($game_user, $slack_channel);
              if ($response !== TRUE) {
                firep($response, 'slack response');
              }
              $response = zg_slack($game_quest, $slack_channel);
              if ($response !== TRUE) {
                firep($response, 'slack response');
              }
              $response = zg_slack($game_equipment, $slack_channel);
              if ($response !== TRUE) {
                firep($response, 'slack response');
              }
            }
          ...
          if ($st_id > 0) {
            ...
            if ($st_success) {
              ...
              zg_slack('could not give 2nd-round st bonus for quest ' . $game_quest->id .
              ...
              $response = zg_slack($game_staff, $slack_channel);
              if ($response !== TRUE) {
                firep($response, 'slack response');
              }
            }
